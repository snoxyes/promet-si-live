#!/usr/bin/env node
/*
 * Promet SI - LIVE GPS dashboard (Node.js port of laguna_dash_gen.py)
 * Multi-operator read-only recon: brezavta.si transit + taxi + micro-mobility.
 *
 * Render: free web service, 24/7 (poll loop keeps it awake).
 *   - regen thread: every INTERVAL s fetches brezavta.si + taxi/micro, writes
 *     laguna_data.json + laguna_dashboard.html into the serving dir.
 *   - http server (process.env.PORT) serves those two files with no-cache headers.
 *
 * Deploy: `git push` to a Render repo connected to render.yaml (or `render` CLI).
 */
...
const SERVE_DIR = __dirname; // server.js lives next to laguna_data.json / html
const CENTER_LAT = 46.0569, CENTER_LON = 14.5058;
const GEN_VER = '2026-08-node-v1';

// CARTO Voyager key (free dev key, public in the original project too).
...
// ── movement cache (persists across polls) ─────────────────────────────────
const _prev = {}; // key -> { hist, spd, spd_hist, good, last_poll_pos, _id, still_pulls }
...
function getJSON(url, headers) {
  return new Promise((resolve) => {
    const u = new URL(url);
    const req = (u.protocol === 'https:' ? https : http).get(url, { headers: headers || {}, timeout: INTERVAL * 1000 + 10000 },
      (res) => {
        let body = '';
...
async function fetchBrezavta() {
  ...
  const WIN = 5;
  // dedup by newest timestamp per id
  const dedup = {};
  for (const x of data) {
  ...
  // reset still-pulls for vehicles absent this poll (Murska Sobota etc.)
  ...
  for (const x of dataArr) {
    ...
    if (sh.length >= 2) {
      ...
      spd = raw != null ? raw : (moved ? st.spd : apiSpd);
    }
    ...
    if (st.still_pulls >= 4) continue; // parked 4 polls (~80s) -> hide
    if (spd != null && spd > 110) spd = 110;
    const rec = {
      Id: v.id || v.gtfs_id,
      Title: v.operator_name || v.operator_id,
      Lat: dlat, Lon: dlon,
      Heading: x.heading,
      Position: true,
      Status: null,
      ...
      __speed__: spd,
      __transit__: {
        operator: v.operator_name || v.operator_id,
        route: x.route_short_name,
...
async function fetchMicromobility() {
  ...
  for (const x of data) {
    ...
    const net = x.network || '?';
    const vid = x.id || net;
    if (mtype !== 'FLOATING') continue; // no static stations
    const mk = 'micro:' + vid;
    const prev = _prev[mk];
    let moved = true;
    if (prev) {
...
// ── HTML (Voyager, same frontend as Python) ────────────────────────────────
...
function tick(){fetch('laguna_data.json?_='+Date.now()).then(function(r){return r.json();}).then(function(d){render(d);if(lastUpd)lastUpd.textContent=new Date().toLocaleTimeString('sl-SI');}).catch(function(e){});}
...
async function writeFiles() {
  ...
  fs.writeFileSync(path.join(SERVE_DIR, 'laguna_data.json'), JSON.stringify(v, null, 0));
  fs.writeFileSync(path.join(SERVE_DIR, 'laguna_dashboard.html'), HTML);
  return v.length;
}
...
const server = http.createServer((req, res) => {
  ...
  if (p === '/' || p === '') p = '/laguna_dashboard.html';
  const file = path.join(SERVE_DIR, path.normalize(p).replace(/^(\.\.[\/\\])+/, ''));
  if (!file.startsWith(SERVE_DIR)) { res.writeHead(403); res.end('forbidden'); return; }
...
// ── start ───────────────────────────────────────────────────────────────────
async function main() {
  // initial write so first load is fresh
  try { await writeFiles(); } catch (e) { console.error('initial write err', e); }
  // regen loop (keeps the service awake on Render — counts as activity)
  setInterval(async () => {
    try { const n = await writeFiles(); console.log(`[regen] ${n} vehicles`); }
    catch (e) { console.error('regen err', e); }
  }, INTERVAL * 1000);
  server.listen(PORT, '0.0.0.0', () => {
    console.log(`[*] Promet SI LIVE dashboard: http://localhost:${PORT}/laguna_dashboard.html`);
  });

  // keep-alive self-ping: Render free tier sleeps after 15 min of inactivity.
  // Hit our own dashboard endpoint every 10 min (from inside the container) so
  // the service never goes idle -> stays 24/7 without an external uptime service.
  const SELF_PING_MS = 10 * 60 * 1000;
  setInterval(() => {
    const url = `http://127.0.0.1:${PORT}/laguna_dashboard.html`;
    http.get(url, (r) => { r.resume(); console.log('[self-ping]', r.statusCode); })
        .on('error', (e) => console.error('[self-ping] err', e.message));
  }, SELF_PING_MS);
  console.log('[self-ping] enabled every 10 min (Render free keep-alive)');
}
main();
